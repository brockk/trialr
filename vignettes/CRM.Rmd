...
| `CrmOneParamLogisticGammaPrior` | $F(x_i, \beta) = 1 / (1 + \exp{(-a_0 - \exp{(\beta)} x_i)})$ | $a_0$ fixed and a gamma prior on $\beta$ |
...
In each parameterisation, the slope parameter $\beta$ is exponentiated to ensure a positive value.
...
We fit the model to our observed data by calling `rstan::sampling`:
...
remain the same. 
However, these estimates look different to the other methods.
We compare in the next section.
...
knitr::kable(
  data.frame(
    DoseLevel = 1:length(skeleton),
    ProbMTD_Empiric = as.numeric(prob_mtd1),
    ProbMTD_Logit2N = as.numeric(prob_mtd4)
  ), digits = 2
)
```

...
This syntax is used in the [EffTox vignette](EffTox.html).
...
This is useful if we `unnest` the data to analyse quantities at the dose-level.
For instance, we might be interested in how our beliefs on the rate of toxicity at dose-level 2 have evolved.
Using functions from `tidyverse` packages, we can proceed:
...
df %>% 
  ...
  filter(dose_index == 2)
```
At each update, the toxicity estimate moves in the direction we would expect in response to the outcomes observed.
...
paths <- crm_path_analysis(
  outcomes_str = '1NNN 2NTN 2NNN 3TTT 1TTT 1TNT',
...
Refer to the [introductory CRM vignette](CRM.html) for more details.
